...
k = 3;
methods = {'llmatcheim', 'llmatchble', 'llmatchapt'};  % 'llmatchpop','llmatcheim', 'llmatcheimfix',
leg = {'EIM', 'BEL', 'HYB'};
np  = length(problems);
nm  = length(methods);
infill_size = 60;
...
tocsv_median = zeros(np, nm);
tocsv_mean = zeros(np, nm);
tocsv_std = zeros(np, nm);
...
for ii = 1:np
    ...
    for jj = 1:nm
       ...
       for kk = 1:seedmax
            ...
            fl = csvread(savename);
            onecollection(kk) = fl(end);
       end
       tocsv_median(ii, jj) = median(onecollection);
       tocsv_mean(ii, jj) = mean(onecollection);
       tocsv_std(ii, jj) = std(onecollection);
...
savename = strcat(foldername,'\matchxl_mean_samexu.csv');
fp=fopen(savename,'w');
...
for ii = 1:np
    ...
    for jj = 1:nm
        ...
        st = strcat(num2str(tocsv_mean(ii, jj)), char(177) , num2str(tocsv_std(ii, jj)));
        fprintf(fp, '%s,', st);
    end
    fprintf(fp, '\n');
end
...
savename = strcat(foldername,'\matchxl_median_samexu.csv');
fp=fopen(savename,'w');
fprintf(fp, 'problem_method, ');
for jj = 1:nm
    fprintf(fp,'%s, ', leg{jj} );
