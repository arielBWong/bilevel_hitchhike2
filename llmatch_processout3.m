...
for ii = 1:np
 ...
 % for each problem plot three methods with error bar
    ...
    t = strcat(prob.name, ' k ', num2str(nv), ' average Kendall corr over 100 samples'); 
    title(t,  'FontSize', 16);
    set(gca, 'XTickLabel',{'20','30','40','50','60','local search'}, 'FontSize', 12);
    legend(hBar, leg{1}, leg{2}, leg{3},  'FontSize', 14); %methods{3},
    
    savename = strcat(pwd, '\result_folder\plots3\', prob.name, '_llmatchperformance_100ken.fig');
    savefig(savename);
    savename = strcat(pwd, '\result_folder\plots3\', prob.name, '_llmatchperformance_100ken.png');
