...
for seed = 1:num_seeds
    ...
    while evaluation < max_evaluation
        ...
        % find the point with the highest EIM value using DE algorithm
        if sum(index) == 0
            infill_criterion = @(x)Infill_Standard_PoF(x, kriging_con);
        else
            switch infill_name
                case 'CEIM_Euclidean'
                    infill_criterion = @(x)Infill_Standard_CEIM_Euclidean(x, kriging_obj, kriging_con, non_dominated_front_scaled);
                case 'CEIM_Maximin'
                    infill_criterion = @(x)Infill_Standard_CEIM_Maximin(x, kriging_obj, kriging_con, non_dominated_front_scaled);
                case 'CEIM_Hypervolume'
                    infill_criterion = @(x)Infill_Standard_CEIM_Hypervolume(x, kriging_obj, kriging_con, non_dominated_front_scaled);
                otherwise
                    error('you should select infill_name from CEIM_Euclidean, CEIM_Maximin, and CEIM_Hypervolume');
            end
        end
        ...
        if min(sqrt(sum((best_x-sample_x).^2,2)))<1E-8
            best_x = DE(@(x)Infill_Maximal_Distance(x, sample_x), num_vari, design_space(1,:), design_space(2,:), 50, 200);
        end
